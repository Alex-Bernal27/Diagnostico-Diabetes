...
def do_admin_login():
    ...
    passForm = passwordLogin
    message_bytes = passForm.encode('ascii')
    base64_bytes = base64.b64encode(message_bytes)
    passwordLoginEncoded = base64_bytes.decode('ascii')
    ...
    with pyodbc.connect('DRIVER='+driver+';SERVER=tcp:'+server+';PORT=1433;DATABASE='+database+';UID='+username+';PWD=' + password) as conn:
        with conn.cursor() as cursor:
            cursor.execute("SELECT TOP (1000) * FROM [dbo].[loginAdmi]")
            row = cursor.fetchone()
            while row:
                usersLogin[row[0]] = row[1]
                row = cursor.fetchone()

    if userNameLogin in usersLogin and usersLogin[userNameLogin] == passwordLoginEncoded:
        account = True
    else:
        account = False
...
def diagnostico():
    ...
    try:
        ...
        cnxn = pyodbc.connect('DRIVER='+driver+';SERVER=tcp:'+server +
